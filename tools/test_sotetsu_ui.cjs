...
(async()=>{
  ...
  try{
    for(const width of [1280,390,320])for(const id of [167,168,169]){
      ...
      assert.equal(await page.locator('#stationList .ressha[data-source="sotetsu"]').count(),id===167?1:2);
      ...
      assert.ok(detail.includes(number),id+': '+detail);
      assert.ok(detail.includes(number==='3002'?'8両':'10両'),detail);
